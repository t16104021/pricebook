...
description: Use when converting sales-history Excel workbooks into the product price management import workbook format in /Users/jimmy/Documents/peactice. Handles 資料庫.xlsx sources with columns such as 銷貨日期(C), 銷貨品號, 品名, 客戶簡稱, 原幣銷貨單價 and outputs 產品, 產品定價, 客戶售價, 使用說明 sheets matching 產品售價管理 templates.
---
...
- `產品定價` = 該 SKU 歷史最高 `原幣銷貨單價`。
- `產品定價.生效日期` = 歷史最高價出現的最新銷貨日期。
- `客戶` = 來源欄位 `客戶簡稱`。
- `客戶售價.生效日期` = 來源欄位 `銷貨日期(C)`。
- `客戶售價` = (`本幣銷貨未稅金額` x `1.05`) / `銷貨數量`。
- `數量` = 來源欄位 `銷貨數量`。
